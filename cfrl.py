...
# Import black box
model = joblib.load('saved_models/df_3_ctgan_model.joblib')

# Import original df
df = pd.read_csv('synth datasets/Dataset_3_CTGAN_synthetic.csv')
...
category_map = {}#1:[0,1]}#, 6:[0,1],7:[0,1]}  #
#9: [0, 1], 10: [0, 1]}

binary_categorical_columns = []#['Gender']#, 'HasCrCard', 'IsActiveMember']#['Tariff Plan', 'Status']
...
feature_types = {'Tariff Plan': int, 'Status' :int}
...
for i, X in enumerate(X_test.values):  # Enumerate directly over the values
